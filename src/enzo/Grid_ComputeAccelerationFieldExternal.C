...
int grid::ComputeAccelerationFieldExternal()
{
  ...
  double MassUnits = 1, sl=1.0e-10;
  //double a_0 = 1.6*3.086e21, rho_0 = 3.3*1.67e-22, c200 = 7.5; //NGC4472
  //double a_0 = 1.2*3.086e21, rho_0 = 5.0*1.67e-22, c200 = 9.0; //NGC5044
  //double a_0 = 20.0*3.086e21, rho_0 = 4.0*1.67e-24, c200 = 5.0; //A2029
  double a_0 = 20.0*3.086e21, rho_0 = 1.0*1.67e-24, c200 = 4.7; //Phoenix
  ...
  if (PointSourceGravity > 0) {
	...
	for (j = 0; j < GridDimension[1]; j++) {
	  ...
	  for (i = 0; i < GridDimension[0]; i++, n++) {
		...
		/*Bondi*/
                ...
                POW(radius*LengthUnits, 2.0) / AccelUnits +
                GravConst*(ClusterSMBHBCG*SolarMass*1.0e11)/POW(radius*LengthUnits +
		EllipticalGalaxyRe*1.0e-3*Mpc_cm/1.8153, 2)/AccelUnits +
                GravConst*SolarMass*ClusterSMBHMass/POW(radius*LengthUnits - 
		2.0*GravConst*SolarMass/POW(clight,2), 2)/ AccelUnits;
                }
              }
              accel = accel/(radius + sl*Mpc_cm/LengthUnits );  // this radius normalizes the multiplication by
    ...
    if (ParticleAcceleration[0] == NULL)
       for (dim = 0; dim < GridRank; dim++) {
           ParticleAcceleration[dim] = new float[NumberOfParticles];
           for (i = 0; i < NumberOfParticles; i++)
               ParticleAcceleration[dim][i] = 0;
    }
	...
	if (PointSourceGravity == 2) {
	  ...
	  accel = accel/( radius + sl*Mpc_cm/LengthUnits);  // this radius normalizes the multiplication by xpos,ypos,zpos done below
  ...
  /  -----------------------------------------------------------  */
      ...
      if (ActiveParticleAcceleration[0] == NULL)
        for (dim = 0; dim < GridRank; dim++) {
            ActiveParticleAcceleration[dim] = new float[NumberOfActiveParticles];
            for (i = 0; i < NumberOfActiveParticles; i++)
                ActiveParticleAcceleration[dim][i] = 0;
        }

      for (i = 0; i < NumberOfActiveParticles; i++) {
         float* ActiveParticleVelocity = ActiveParticles[i]->ReturnVelocity();
     /* Compute vector between active particle (advanced by 1/2 step) and gravity center. */
         xpos = ActiveParticlePosition[0][i] + 0.5*dtFixed*ActiveParticleVelocity[0]/a -
           PointSourceGravityPosition[0];
         ypos = ActiveParticlePosition[1][i] + 0.5*dtFixed*ActiveParticleVelocity[1]/a -
           PointSourceGravityPosition[1];
         zpos = ActiveParticlePosition[2][i] + 0.5*dtFixed*ActiveParticleVelocity[2]/a -
           PointSourceGravityPosition[2];
         ...
         if (PointSourceGravity == 2) {
         /* NFW Profile: assume CoreRadius is rs in cm and Constant is mass within rs in g. */
             radius = sqrt(rsquared);
 	     FLOAT x = radius/rcore;
             accel = GravConst*PointSourceGravityConstant*MassUnitsDouble*
               ((log(1.0+x  )-x  /(1.0+x  )) /
                 (log(1.0+c200)-c200/(1.0+c200))) /
               POW(radius*LengthUnits + sl*Mpc_cm, 2) / AccelUnits;
		   ...
		   - 2.0*GravConst*SolarMass*ClusterSMBHMass/pow(clight,2), 2) / AccelUnits;
             ...
             } // End if (ProblemType == 108)
             accel = accel/(radius+sl*Mpc_cm/LengthUnits);
          } // End if (PointSourceGravity == 2)
         ActiveParticleAcceleration[0][i] -= 0.0; //Keeping the AGN Particle at a fixed position
         ActiveParticleAcceleration[1][i] -= 0.0;
	 ActiveParticleAcceleration[2][i] -= 0.0;
      } // End of loop of number of Active Particles
     } // End if for Active Particles
   /* ------------------- End ACTIVE PARTICLE --------------------  /
   / --------------------------------------------------------------  */
