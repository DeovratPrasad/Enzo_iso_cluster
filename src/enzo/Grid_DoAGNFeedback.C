...
/* This bypass was introduced by DP to account for Bondi accretion in addition to 
 * cold gas accretion rate for test purposes. This should be switched off for cold mode
 * aaccretion or pure Bondi accretion rate as they are accounted for above. 
 */
   float mdot_bondi;
   if (AGNParticleBondiAccretion == 1){
      mdot_bondi = AGNParticleGetBondiMassRate(ThisParticle);
      printf("Calculating Bondi accretion rate. \n");
   } else  {
      mdot_bondi = 0.0;
   }
   mdot =  mdot + mdot_bondi;
